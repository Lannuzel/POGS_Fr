...
function createOrRetrieveGroupPad(groupID,initialText, padNamed) {
    var padName = (padNamed)?(padNamed):(uuidv4());
    // Vérifiez si le pad existe déjà
    if (padExists(padName)) {
        return padName; // Retourne le pad existant
    }

    // Si le pad n'existe pas, créez-le
    var apiResponse = JSON.parse(newConnection(
        API_ADDRESS + "createGroupPad/?apikey=" + API_KEY + "&groupID=" + groupID + "&padName=" + padName + "&text=" + initialText,
        "GET"
    ));
    if (apiResponse && apiResponse.data && apiResponse.data.padID) {
        print(apiResponse.data.padID);
        return apiResponse.data.padID;
    }
    return null;
...
function createSession(groupID, authorID) {
    var validUntil = Math.floor(new Date().getTime() / 1000) + (365 * 24 * 60 * 60); // 1 an en secondes
    var apiResponse = JSON.parse(newConnection(
        API_ADDRESS + "createSession/?apikey=" + API_KEY + "&groupID=" + groupID + "&authorID=" + authorID + "&validUntil=" + validUntil,
        "GET"
    ));
    if (apiResponse && apiResponse.data && apiResponse.data.sessionID) {
        return apiResponse.data.sessionID;
    }
    return null;
}
...
function padExists(padName) {
    var apiResponse = JSON.parse(newConnection(
        API_ADDRESS + "getRevisionsCount/?apikey=" + API_KEY + "&padID=" + padName,
        "GET"
    ));
    return apiResponse && apiResponse.code === 0; // Retourne true si le pad existe
}
...
//     document.cookie = "sessionID=" + sessionID + "; path=/; HttpOnly";
// }
...
if (!padID) {
    // Aucun pad encore créé pour cette tâche : on en crée un nouveau.
    padID = createOrRetrieveGroupPad(groupId, "", null);
    attributesToAddz = [{
        "attributeName": "padID",
        "stringValue": padID
    }];
...
// Si padID existe déjà, tous les joueurs utilisent le même pad : rien à faire.
