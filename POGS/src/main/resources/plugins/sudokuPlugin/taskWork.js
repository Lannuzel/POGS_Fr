class SudokuGame {
        ...
        $(".sudoku__gametable-cell").each(function() {
            ...
            if(cellIndex <= gridBluePrintCells.length) {
                if(gridBluePrintCells[cellIndex] == 0 ){
                    ...
                    $(this).attr("inputmode", "numeric"); // Ajouter cet attribut pour le pavé numérique
    ...
    handleKeydown(theEvent){
        ...
        if ([8, 9, 46, 37, 38, 39, 40].includes(key)) {
            return true;
        }
        ...
        // Check for numeric input (standard keys and numpad keys)
        let isNumeric = (key >= 48 && key <= 57) || (key >= 96 && key <= 105); // Standard and numpad numbers
        if (!isNumeric) {
            theEvent.returnValue = false;
            theEvent.preventDefault();
            return false;
