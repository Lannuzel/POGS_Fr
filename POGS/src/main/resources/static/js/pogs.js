...
class Pogs {
    ...
    setup (config) {
        ...
        if(!(window.location.href.indexOf("/waiting_room")>=0)&&!(window.location.href.indexOf("/sessions/start/")>=0)) {
            ...
            //     text: 'Next',
            //     class: 'next-button',
            // }).appendTo(".headers"); // Assurez-vous que ".headers" est le bon sélecte
        }
        //console.log("Teammates:", this.teammates);
        //console.log("Number of teammates:", this.teammates.length);    
        ...
        //     // Envoyer une notification au serveur pour signaler que le joueur a cliqué
        //     fetch("/api/playerClicked", {
        //         method: "POST",
        //         headers: {
        //             "Content-Type": "application/json",
        //         },
        //         body: JSON.stringify({ playerId: this.subjectId }), // Ajoutez un `playerId` unique par joueur
        //     })
        //         .then((response) => response.json())
        //         .then((data) => {
        //             console.log("Joueur enregistré :", data);
        //         })
        //         .catch((error) => console.error("Erreur de synchronisation :", error));
        // });

        // setInterval(() => {
        //     this.checkAllPlayersReady(); // Vérifiez périodiquement si tous les joueurs ont cliqué
    ...
    onFlowBroadcastReceived(message) {
       ...
       console.log("this.isGlobalChatPage: " + this.isGlobalChatPage)
        ...
        if(this.sessionIsPerpetual && message.content.perpetualSubjectsChosen) {
    ...
    sendMessage(url, type, messageContent, sender, receiver, completedTaskId, sessionId) {
