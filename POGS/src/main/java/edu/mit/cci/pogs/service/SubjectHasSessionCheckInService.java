...
public class SubjectHasSessionCheckInService {
    ...
    static final Integer ACCEPTABLETIMETHRESHOLD = 60*1000;
    ...
    public void updateLatestSubjectPing(Long subjectId, Long sessionId) {
        ...
        if(shsc != null) {
            ...
            // Si le joueur avait été marqué perdu mais repinge, on réinitialise son flag
            // pour qu'il réapparaisse dans listReadyToJoinSubjects
            if (Boolean.TRUE.equals(shsc.getHasLostSession()) && !Boolean.TRUE.equals(shsc.getHasJoinedSession())) {
                shsc.setHasLostSession(false);
                shsc.setLostSubjectTime(null);
            }
            subjectHasSessionCheckInDao.update(shsc);
