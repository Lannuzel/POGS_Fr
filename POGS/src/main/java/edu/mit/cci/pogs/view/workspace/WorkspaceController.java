...
public class WorkspaceController {
    ...
    public String landingPageLoginPost(@PathVariable("sessionId") String sessionId,
        ...
        if (alreadyHasCookie) {
            ...
            if (su != null) {
                ...
                // sr peut être null si POGS a redémarré depuis le dernier check-in du joueur
                boolean isPerpetual = (sr != null) && sr.getSession().isSessionPerpetual();
                model.addAttribute("pogsSessionPerpetual", isPerpetual);
                // Re-check in the subject so they appear in the perpetual runner queue
                if (isPerpetual) {
    ...
    public String register(@RequestParam("externalId") String externalId, Model model) {
        ...
        SessionRunner sr = SessionRunnerManager.getSessionRunner(su.getSessionId());
        if (sr == null) {
            // Sub-session runner may not be started yet (perpetual clone race). Force init and retry once.
            sessionService.initializeSessionRunners();
            sr = SessionRunnerManager.getSessionRunner(su.getSessionId());
        }
        if (sr == null) {
