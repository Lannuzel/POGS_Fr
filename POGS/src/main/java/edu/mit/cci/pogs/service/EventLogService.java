        ...
        if (attributesToAddJson != null) {
            ...
            if (array != null) {
                for (int i = 0; i < array.length(); i++) {
                    ...
                    if (jo.has("eventType") && !jo.isNull("eventType")) {
                        eventLog.setEventType(jo.getString("eventType"));
                    }
                    if (jo.has("timestamp") && !jo.isNull("timestamp")) {
                        eventLog.setTimestamp(new Timestamp(jo.getLong("timestamp")));
                    }
                    if (jo.has("eventContent") && !jo.isNull("eventContent")) {
                        eventLog.setEventContent(jo.getString("eventContent"));
                    }
                    if (jo.has("summaryDescription") && !jo.isNull("summaryDescription")) {
