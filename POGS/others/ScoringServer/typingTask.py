...
def calculate_text_does_not_have_ground_truth(request_parameters):
	...
	if request_parameters['completedTaskAttributes'][0] != None and request_parameters['dictionaryEntries'][0] != None:
		...
		# typed_text peut être None si fullText est absent des attributs
		if typed_text is None:
			typed_text = ''

		# Ignorer les lignes vides et dédoublonner (casse insensible)
		seen = set()
		answers_lines = []
		for l in typed_text.splitlines():
			normalized = l.lower().strip()
			if normalized and normalized not in seen:
				seen.add(normalized)
				answers_lines.append(l)
		total_entries = len(answers_lines)
		all_dictionary_entries = dictionary_entries

		print("Answser has : " + str(total_entries) + " entries")
				...
				for ent in list(category_entries):
					if answer.lower().strip() == ent.lower().strip():
						...
						if dict_entries['entryType'] == 'C':
							...
							right_categories_found[dict_entries['entryCategory']] = \
								right_categories_found.get(dict_entries['entryCategory'], 0) + 1
						break
				if found:
					break
			if not found:
	...
	# totalScore = nombre de mots corrects (type C reconnus)
	# scoringData conserve aussi le nombre de catégories distinctes trouvées
	final_score = correct_words_count
	distinct_categories = len(right_categories_found)

	resp = { 'completedTaskScore': {
		'numberOfProcessedEntries' : entries_processed,
		'numberOfWrongAnswers' : len(not_found_entries),
		'numberOfRightAnswers' : correct_words_count,
		'completedTaskId' :  request_parameters['completedTaskId'][0],
		'numberOfEntries' :  total_entries,
		'totalScore' : final_score,
		'scoringData' : json.dumps({'distinct_categories': distinct_categories,
		                        'categories_detail': right_categories_found})
