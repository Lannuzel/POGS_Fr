...
tmux send-keys -t my_session:0 'cd ./POGS; java -jar target/pogs-0.0.1-SNAPSHOT.jar' C-m

# Créer une nouvelle fenêtre (1) dans la même session pour la deuxième commande
tmux new-window -t my_session:1
tmux send-keys -t my_session:1 'cd ./etherpad-lite; ./bin/run.sh --root' C-m

# Créer une nouvelle fenêtre (2) dans la même session pour la deuxième commande
tmux new-window -t my_session:2
tmux send-keys -t my_session:2 'cd ./POGS/others/ScoringServer; python3 server.py' C-m
